...
iperf_tcp_recv(struct iperf_stream *sp)
{
    ...
    if (sp->test->state == TEST_RUNNING) {
        ...
        if (sp->test->data_integrity && r > 0) {
            ...
            while (pos < r) {
                int block_remaining = blksize - sp->data_integrity_block_offset;
                int chunk = (r - pos < block_remaining) ? (r - pos) : block_remaining;

                /* Process header bytes (offsets 0-7 of each block) */
                if (sp->data_integrity_block_offset < 8) {
                    int hdr_start = sp->data_integrity_block_offset;
                    int hdr_bytes = (hdr_start + chunk > 8) ? (8 - hdr_start) : chunk;
                    memcpy(sp->data_integrity_header_buf + hdr_start,
                           sp->buffer + pos, hdr_bytes);

                    /* If there are payload bytes in this chunk (past byte 8), add to CRC */
                    if (sp->data_integrity_block_offset + chunk > 8) {
                        int payload_start = 8 - sp->data_integrity_block_offset;
                        if (payload_start < 0) payload_start = 0;
                        sp->data_integrity_running_crc = iperf_crc32_update(
                            sp->data_integrity_running_crc,
                ...
                sp->data_integrity_block_offset += chunk;
                pos += chunk;

                /* Block complete -- validate */
                if (sp->data_integrity_block_offset >= blksize) {
                    uint32_t recv_seq, recv_crc;
                    memcpy(&recv_seq, sp->data_integrity_header_buf, sizeof(recv_seq));
                    memcpy(&recv_crc, sp->data_integrity_header_buf + 4, sizeof(recv_crc));
                    recv_seq = ntohl(recv_seq);
                    recv_crc = ntohl(recv_crc);

                    uint32_t computed_crc = iperf_crc32_finalize(sp->data_integrity_running_crc);
                    ...
                    if (recv_seq != sp->data_integrity_block_seq) {
                        iperf_err(sp->test,
                            "DATA INTEGRITY ERROR on stream %d: sequence mismatch - "
                            "expected %u, got %u",
                            sp->socket, sp->data_integrity_block_seq, recv_seq);
                        i_errno = IEDATAINTEGRITY;
                        sp->test->data_integrity_error = 1;
                        sp->test->done = 1;
                        return -1;
                    }
                    sp->data_integrity_block_seq++;
                    sp->data_integrity_block_offset = 0;
                    sp->data_integrity_running_crc = IPERF_CRC32_INIT;
                }
            }
        }
    }
...
iperf_tcp_send(struct iperf_stream *sp)
{
    ...
    if (!sp->pending_size) {
        ...
        if (sp->test->data_integrity) {
            uint32_t seq = htonl(sp->data_integrity_block_seq++);
            uint32_t crc = htonl(sp->data_integrity_payload_crc);
