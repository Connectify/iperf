            ...
            if (sp->sender == stream_must_be_sender) {
                ...
                if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                    ...
                    if (test->json_output) {
                        ...
                        if (test->data_integrity)
                            cJSON_AddNumberToObject(json_udp, "data_integrity_errors", (double) sp->data_integrity_errors);
                ...
                if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                    ...
                    if (! test->json_output) {
                        ...
                        if (test->role == 's' && sp->sender) {
                            ...
                            if (test->data_integrity && sp->data_integrity_errors > 0)
                                iperf_printf(test, report_data_integrity_errors, sp->socket, mbuf, start_time, receiver_time, (uint64_t) sp->data_integrity_errors);
                        }
                    }
                }
            }
...
iperf_new_stream(struct iperf_test *test, int s, int sender)
{
    ...
    if (test->data_integrity) {
        sp->data_integrity_block_seq = 0;
        sp->data_integrity_block_offset = 0;
        sp->data_integrity_running_crc = IPERF_CRC32_INIT;
        sp->data_integrity_errors = 0;
        memset(sp->data_integrity_header_buf, 0, sizeof(sp->data_integrity_header_buf));
        ...
        if (sender && test->protocol->id != Pudp) {
            ...
            sp->data_integrity_payload_crc = iperf_crc32(
                sp->buffer + 8,
